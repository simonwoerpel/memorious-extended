...
from memorious.operations.store import _get_directory_path as get_directory_path
...
def get_zavod(context: Context) -> Zavod:
    data_path = Path(get_directory_path(context))
...
def make_document(context: Zavod, data: Data, config: Data) -> CE:
    ...
    proxy.add("fileName", data["url"].split("/")[-1])
